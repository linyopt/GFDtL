...
banded_exps(T, p, num_breaks, num_exps, lamb1s, lamb2s, lamb3, epsilon, beta_, tau, Nsim, gfgl)
%% Run experiments for banded settings.
% Obtain estimators (both D-trace and GFGL) based on several simulated
% ar datasets and compare the performance.
%
% - Usage:
...
%          gresHFE, gparaHFE, gresBIC, gparaBIC, gresloss, gparaloss, ...
%          EstHFE, EstBIC, Estloss, ThGHFE, ThGBIC, ThGloss] = ...
% banded_exps(T, p, num_breaks, prob, num_exps, lamb1s, lamb2s, lamb3, epsilon, beta_, tau, Nsim, gfgl);
%
% - Input:
...
for it = 1:num_exps
    ...
    [Y(:, :, it), Y_test(:, :, :, it), trueTheta(:, :, :, it), ...
            trueBreaks(:, it)] = DGP_banded(T, p, num_breaks, tau, Nsim);
