...
iid_exps(T, p, num_breaks, prob, num_exps, lamb1s, lamb2s, lamb3, epsilon, beta_, tau, Nsim, gfgl)
%% Run experiments for iid settings.
% Obtain estimators (both D-trace and GFGL) based on several simulated
% iid datasets and compare the performance.
%
% - Usage:
...
for it = 1:num_exps
    ...
    [Y(:, :, it), Y_test(:, :, :, it), trueTheta(:, :, :, it), ...
            trueBreaks(:, it)] = DGP_iid(T, p, num_breaks, prob, tau, Nsim);
