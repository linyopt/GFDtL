function [hE,hV]=wgPlotlw(adjMat,coord,varargin)
...
%
% Modified by Alex Gibberd (April 2015) in order to vary edge width according to size of
% edge value in adjacency matrix
%
%====================
...
% axesArea(h,[6 7 5 5]);    % allows to be used within subplot()..
plotParm={'markerSize',6,'marker','.','MarkerEdgeColor',[1,0.5,0.2]};
siz=size(adjMat);
vrtxSiz=100;
edgeMap=cool;
vrtxMap=summer;
leg=0;  % Default not to plot legend
...
for kk=1:2:nVarArgin
    ...
    case 'vertexscale'
      vrtxSiz=varargin{kk+1};
        case 'legend'   % Added option for specifying whether should plot legend
            leg=varargin{kk+1};
    otherwise
...
qq=unique([ii,jj]);
qq=[1:length(adjMat)];  % use all vertices..
% minEWt=min(eWt);
minEWt=0;
% maxEWt=max(eWt);
maxEWt=1.3;   % Fix scale for GFGL
...
if eWeighted
  ...
  for kk=1:neColor
    ...
    hE=[hE,plot(x(:),y(:),'lineWidth',10*(kk/neColor),'color',edgeMap(kk,:),plotParm{:})];
    % Draw legend bar
    if(leg)
