function [ Theta,Z,cp,S,aditer,fobj ] = GFGLdiag( y,lambda1,lambda2,gamma )
%ADMM to solve Group Fused Graphical lasso but without BCD
% This version has dykstra iterative projection for subproblem
% Alex Gibberd - UCL
% This version also smoothes diagonal...to see if we can recover standard
% stationary estimators.
...
%    end
    ...
    % Puts edges AND diagonal in matrix
    for i=1:P
        for j=i:P
            Thetam(k,t)=Theta(i,j,t);
            k=k+1;
