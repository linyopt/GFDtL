function [ Y, Y_test, Theta_tt, breaks ] = DGP_Erdos_Renyi(T, p, m, prob, tau, Nsim, regimes)
%% Generate Erdos Renyi data set.
%
% [Y, Y_test, Theta, breaks] = DGP_Erdos_Renyi(T, p, m, prob, regimes);
%
% - Input:
%   @T:         Sample size.
%   @p:         Dimension.
...
%   @tau:       The rescaling factor of Theta, default 14.
...
for k = 1:m+1
        ...
        for jj=1:p-1
            for ll=jj+1:p
                ...
                while cond_dist
                    param = a+(b-a)*rand(1);
                    cond_dist=(param>-limit & param<limit);
                end
                Theta_temp(jj,ll)=param;
            end
        end
        Theta_temp = Theta_temp + Theta_temp'; c = 0.5; d = 1; % c = 4; d = 6;
        Theta(:,:,k)=Theta_temp.*adjacency + diag(c+(d-c)*rand(p,1));
        if min(eig(Theta(:,:,k)))<0.01
            zeta = 0;
            while (min(eig(Theta(:,:,k)))<0.01)
                Theta(:,:,k) = Theta(:,:,k) + (zeta + abs(min(eig(Theta(:,:,k)))))*eye(p);
                zeta = zeta + 0.005;
            end
        end

end
% Apply Theta = Theta/tau to get smaller entries, which will give larger Y.
Theta = Theta/tau;
